...
| UP14 | Backpropagation-free training (MeZO/ZO methods) can fine-tune LLMs with forward passes only | MeZO (Princeton NLP, NeurIPS 2023), MobiZO (EMNLP 2025), ElasticZO (Jan 2025). MeZO: 2-25× memory reduction. | **✅ CONFIRMED (2026-03-16)**: MeZO+LoRA-split on SmolLM2-360M achieves 262ms/step = 1.71x faster than CPU. Conv-fused kernels reduce IO from 224→96 trips. FZOO (K=4) and P-GAP provide no wall-time convergence benefit for LoRA ZO. | CONFIRMED |
