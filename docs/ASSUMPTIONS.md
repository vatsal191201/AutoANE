...
| UP19 | ACCUM_STEPS=100 gives 4.74x throughput improvement | maderix/ANE Issue #24. At stories15M: 0.66 steps/s (accum=1) → 3.15 steps/s (accum=100). Amortizes compile overhead across more forward/backward passes. | We already use accum=7-10. Higher values could significantly improve effective throughput for larger models. | HIGH |
